package com.dsy.dsu.CoreApp.Apps.BootAndAsyncApp.Model.BinesslogicActivityBoot;
...
import com.dsy.dsu.CoreApp.Apps.BootAndAsyncApp.Model.DowloadUpdatePO.DownLoadPO;
...
import com.dsy.dsu.CoreApp.Apps.BootAndAsyncApp.View.BootFragment;
...
import com.dsy.dsu.CoreApp.Apps.ErrorsCoreApp.model.bl_fragmenterrorlautch.LaunchFragmenrError;
import com.dsy.dsu.R;
import com.dsy.dsu.CoreApp.Apps.SettingsCoreApp.View.MainActivity_Settings;
...
// TODO: 10.07.2023  запуск обновление ПО
                                    ...
                                    new CreateSingleWorkManager(context).getcreateSingleWorkManager("lanchOnlyUpdatePO");//"lanchUpdatePO"   // TODO: 03.03.2025 Запускем Синхрнонизацию через Singlw Work manager
    ...
    public void registeEventBusFirst(@NonNull BootFragment bootFragment) throws Exception {

        if (  !EventBus.getDefault().isRegistered(bootFragment)) {
            EventBus.getDefault().register(bootFragment);
        }
        Log.d(bootFragment.getContext().getClass().getName(), "\n"
                + " время: " + new Date() + "\n+" +
                " Класс в процессе... " + this.getClass().getName() + "\n" +
                " метод в процессе... " + Thread.currentThread().getStackTrace()[2].getMethodName()+
                " registeEventBusFirst bootFragment  " +bootFragment);
    }

    public void unregisterEventBusFirst(@NonNull BootFragment bootFragment) throws Exception {
        if (  EventBus.getDefault().isRegistered(bootFragment)) {
            EventBus.getDefault().unregister(bootFragment);
        }
        Log.d(bootFragment.getContext().getClass().getName(), "\n"
                + " время: " + new Date() + "\n+" +
                " Класс в процессе... " + this.getClass().getName() + "\n" +
